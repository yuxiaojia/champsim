...
if [ "$#" -lt 5 ] || [ "$#" -gt 6 ]; then
    ...
    echo "Usage: ./run_1core.sh [BINARY] [N_WARM] [N_SIM] [N_MIX] [TRACE0] [TRACE1] [TRACE2] [TRACE3] [OPTION]"
    exit 1
fi
...
(./bin/${BINARY} -warmup_instructions ${N_WARM}000000 -simulation_instructions ${N_SIM}000000 ${OPTION} -traces ${TRACE_DIR}/${TRACE0}) &> results_1core_${N_SIM}M/mix${N_MIX}-${BINARY}${OPTION}${TRACE0}.txt